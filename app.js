...
(function () {
  ...
  RW.userProfile = (username) => {
    ...
    // Wishlist: use the curated one if we have it, else derive a stable pair.
    let wishlist = D.WISHLISTS[username];
    if (!wishlist) {
      const types = D.WISH_TYPES, sizes = D.WISH_SIZES;
      wishlist = [
        { type: types[h % types.length], size: sizes[(h >>> 3) % sizes.length] },
        { type: types[(h >>> 5) % types.length], size: sizes[(h >>> 7) % sizes.length] },
      ];
    }
    return {
      username,
  ...
  function generateReply(text, convo) {
    ...
    // You stating what you want (answering "what are you looking for?").
    // Checked first so it isn't read as you asking THEM what they want.
    if (/\b(i'?m looking for|i'?m after|hoping to find|i'?d love to find|on my wishlist|searching for|anything good quality|anything (in )?my size|i need (some|a |to find))\b/i.test(t))
      return "Nice, that helps! 🌱 I might have a few bits that fit the bill, ask me what I've got and I'll send pics!";

    // shared logistics + closing rules (same regardless of role)
    const common = [
      [/\b(already (been )?swapped|already (gone|traded|claimed)|no longer available|it'?s gone|been taken|someone else got)\b/,
        "Ah that's a shame! 😅 No worries though, thanks for letting me know 🌱 Happy swapping!"],
      [/\b(where|meet|meetup|location|place|spot|pick ?up|drop ?off|come by|campus)\b/,
        "How about the library or the Nepean Sportsplex? 📍 Both are easy for me. Which works better for you?"],
  ...
  /* You telling someone the item is already gone. */
  const isAlreadySwapped = (text) =>
    /\b(already (been )?swapped|already (gone|traded|claimed)|no longer available|it'?s gone|been taken|someone else got)\b/i.test(text);

  /* You asking what the other person has to offer. */
  const isWhatDoYouHave = (text) =>
    /\bwhat (do you have|have you got|can you (offer|trade)|you got|else do you have|do you have to (swap|trade|offer)|are you offering)\b/i.test(text) ||
    /\bwhat'?s? (in your closet|on offer|up for (swap|trade))\b/i.test(text) ||
    /\bshow me what you('?ve)? (have|got)\b/i.test(text);

  /* They reach into the "image bank" and offer a real piece, with a photo. */
  function simulateItemOffer(convo, delay = 1700) {
    ...
    setTimeout(() => {
      ...
      const pick = D.IMAGE_BANK[Math.floor(Math.random() * D.IMAGE_BANK.length)];
      const text = `I have a ${pick.name} I'd be happy to swap! Here's a pic 📸`;
      convo.messages.push({ from: "them", text, time: "Now" });
      convo.messages.push({ from: "them", type: "image", src: pick.image, time: "Now" });
      convo.time = "Now";
      convo.offeredItem = pick.name;
      if (isViewingConvo(convo)) {
        const b = document.getElementById("bubbles");
        if (b) { appendBubble(b, "them", text); appendImageBubble(b, "them", pick.image); }
      } else {
        convo.unread = (convo.unread || 0) + 2;
  ...
  function sendChat(text) {
    ...
    if (isWhatDoYouHave(msg)) { simulateItemOffer(c); return; }
    // Telling someone it's gone: they bow out and won't reach out again.
    if (isAlreadySwapped(msg) && c.user) dismissedUsers.add(c.user);
  ...
  function wishlistModal() {
    if (!state.auth) { toast("Sign in to build a wishlist"); return; }
    openModal(`
      <h3>Add to wishlist</h3>
      <p class="muted tiny">Tell swappers what you're hoping to find.</p>
      <div class="field"><label>Clothing type</label>
        <select class="input" id="wishType">
          ${D.WISH_TYPES.map((t) => `<option value="${t}">${t}</option>`).join("")}
        </select>
      </div>
      <div class="field"><label>Size</label>
        <select class="input" id="wishSize">
          ${D.WISH_SIZES.map((s) => `<option value="${s}">${s}</option>`).join("")}
        </select>
      </div>
      <button class="btn btn-primary mt-16" id="confirmWish">Add to wishlist</button>
      <button class="btn btn-ghost mt-12" data-close-modal>Cancel</button>
    `);
  }

  function addWishlist() {
    const type = document.getElementById("wishType").value;
    const size = document.getElementById("wishSize").value;
    state.user.wishlist = state.user.wishlist || [];
    if (state.user.wishlist.some((w) => w.type === type && w.size === size)) {
      toast("Already on your wishlist");
      closeModal();
      return;
    }
    state.user.wishlist.push({ type, size });
    persistSession();
    closeModal();
  ...
  function removeWishlist(i) {
    if (!state.user.wishlist) return;
    state.user.wishlist.splice(i, 1);
    persistSession();
    render("profile");
  }
  ...
  /* People you've told "already swapped" — they won't reach out again. */
  const dismissedUsers = new Set();

  function pickPersona() {
    const me = (state.user.username || "").toLowerCase();
    const pool = PERSONAS.filter((p) => p !== me && !dismissedUsers.has(p));
  ...
  function scheduleInterest(item) {
    ...
    setTimeout(() => {
      ...
      if (persona) deliverIncoming(item, persona, openerFor(item));
    }, delay);
  }
  ...
  document.addEventListener("click", (e) => {
    const t = e.target.closest("[data-nav],[data-open],[data-save],[data-save-btn],[data-back],[data-filter],[data-onb],[data-auth],[data-theme-toggle],[data-chat],[data-request],[data-delete],[data-close-modal],[data-logout],[data-quick],[data-match-quick],[data-match-arrange],[data-rate],[data-star],[data-rtag],[data-user],[data-msg-user],[data-mark-swapped],[data-goal],[data-link],[data-wish-add],[data-wish-del]");
    if (!t) return;
    ...
    if (t.hasAttribute("data-wish-add")) { wishlistModal(); return; }
    if (t.dataset.wishDel != null) { removeWishlist(parseInt(t.dataset.wishDel, 10)); return; }
  ...
  document.addEventListener("click", (e) => {
    ...
    if (e.target.id === "confirmWish") addWishlist();
  });
