...
(function () {
    ...
    return `
      ...
      <section class="card glow rise mt-16" style="padding:18px;overflow:hidden">
        <div class="row between">
          ...
          <div class="sdg" style="width:54px;aspect-ratio:1;padding:8px">
            <div class="n" style="font-size:18px">12</div>
            <div class="t" style="font-size:7px">Responsible<br>Consumption</div>
          </div>
        </div>
  ...
  Screens.chat = () => {
    ...
    return `
      ...
      <div class="scroll-x quickbar">
        ${D.QUICK_REPLIES.map((q) => `<button class="chip" data-quick="${q}">${q}</button>`).join("")}
      </div>
  ...
  Screens.match = () => {
    ...
    const mine = m.mineTone != null ? m.mineTone : 150;
    return `
      <div class="match-wrap">
        <div class="confetti" id="confetti"></div>
        <div class="chip chip-green chip-static">Swap complete</div>
        <div class="match-cards">
          ${imgPh("Your item", "tall", mine, m.mineImage).replace("imgph tall", "imgph tall mc1")}
